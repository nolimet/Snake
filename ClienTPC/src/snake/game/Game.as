package snake.game
{
	...
	public class Game extends Sprite 
	{
		...
		private function checkColl():void {
			if (intersectsTest(pickUp,Player.square)) {
				removeChild(pickUp);
				addPickUp();
				Player.addBlock();
			}
			else if (intersectsTest(pickUp,Player2.square))  {
				removeChild(pickUp);
				addPickUp();
				Player2.addBlock();
			}
			for (var i:int = 0; i < Player.squares.length; i++) {
				if (intersectsTest(Player2.square,Player.squares[i])) {
					trace("Player2hit");
					Resett();
				}
				if (intersectsTest(Player.square,Player.squares[i]) && i < Player.squares.length - 2) {
					trace("Player1hitself");
					Resett();
				}
			}
			for (var j:int = 0; j < Player2.squares.length; j++) {
				if (intersectsTest(Player.square,Player2.squares[j])) {
					trace("Player1hit");
					Resett();
				}
				if (intersectsTest(Player2.square,Player2.squares[j]) && j < Player2.squares.length - 2) {
